...
public class RuntimeUtil {
     ...
     */
    private static void runAsUnblocking(final RuntimeCallback callback, final String[] envp, final File dir,
     ...
     * @return 命令执行后的最终结果
     */
    private static StringReader run(String[] envp, File dir, String[] startCommand, String[]... commands) {
        // 生成一个命令id，只是为日志区别用
        String commandId = UUID.randomUUID().toString();
//        WsClient.get().send("Command(" + commandId + ") start");
...
//        WsClient.get().send("Command(" + commandId + ") end");
        // 用结果缓冲构建StringReader
     ...
     * @return
     */
    private static String readStringReader(StringReader stringReader, boolean skipWhiteLine) {
        StringBuilder result = new StringBuilder();
...
//        WsClient.get().send("Command(" + commandId + ") exec: " + Arrays.toString(command));
     ...
     * @author fox
     */
    private static interface RuntimeCallback {
        /**
         * 非阻塞模式回调函数
         *
         * @param result       命令执行结果
    ...
    public static void main(String[] args) {
        WsClient.get().send("cmd:ls -a");
    // 例子 ps -ef (注意用String[]方式来表示命令是不需要做转移的)
        readStringReader(run(null, null, new String[]{"cd "," .."}), true);
        String result = readStringReader(run(null, null, new String[]{"pwd"}), true);
        WsClient.get().send("执行结果如下:" + result);
    // System.out.println("-----------------------------------------------------------------");
...
//                run(null, null, new String[]{"ps", "-ef"}, new String[]{"grep", "-i", "java"}), true));
    ...
    public static void exeCommadn(String[] args) {
        String result = readStringReader(run(null, null, args), true);
        WsClient.get().send("执行结果如下:\n" + result);
    }
}